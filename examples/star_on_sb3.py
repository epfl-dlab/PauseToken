...
from lm_stable_baselines.callbacks.lm_callback import LMCallback

def create_env(*args, **kwargs):
    return LanguageModelEnv(*args, **kwargs)
...
if __name__ == "__main__":
    ...
    dataset = {"train": load_dataset('json', data_files=input_dir + 'train.json', split='train')}
    dataset = dataset["train"].rename_column("question", "input_text").rename_column("answer", "output_text")
    ...
    generation_config = GenerationConfig(pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id, max_length=1024)
    
    policy = LLMBasePolicy
    policy_kwargs = {"lm": model, "tokenizer": tokenizer,"generation_config": generation_config}
    ...
    algo.learn(10, callback=LMCallback())
