...
import torch
from src.utils.trainer_utils import decode_and_strip_special_tokens, extract_math_answer, are_latex_expressions_equal, INVALID_ANS

class MathCorrectnessReward(AbstractReward):
    ...
    def reward_fn(self, model_output: torch.LongTensor, ground_truth: torch.LongTensor):
        ...
        gt_answer = extract_math_answer(decode_and_strip_special_tokens(ground_truth,self.tokenizer))
        assert gt_answer != INVALID_ANS, f"Ground truth answer is invalid, your ground truth answer is {decode_and_strip_special_tokens(model_output,self.tokenizer)}"
        #extract the answer of the model output
        pred_answer = extract_math_answer(decode_and_strip_special_tokens(model_output,self.tokenizer))
