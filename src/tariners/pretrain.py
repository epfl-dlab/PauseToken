...
def main():
    ...
    if "/" in model_name :
        output_directory =f'{args.output_dir}/{task}/{args.tag}/pretrain_{model_name.split("/")[-1]}_trl_{datetime.now()}'
    else: 
        output_directory =f'{args.output_dir}/{task}/{args.tag}/pretrain_{model_name}_trl_{datetime.now()}'
    ...
    trainer = SFTIgnoreTokensInLossTrainer(
        ignore_tokens = [pause_token_id],
        model=model,
        tokenizer=tokenizer,
        args=training_args,
    ...
    trainer_to_save = SFTIgnoreTokensInLossTrainer(
        ignore_tokens = [pause_token_id],
        model=model,
        tokenizer=tokenizer,
        args=training_args,
    ...
    trainer_to_save.save_model(args.output_dir)
