...
class LanguageModelEnv(Env):
    ...
    def reset(
        ...
        self.output_text = input_sample["output_text"]
        
        self.current_state = self.tokenizer(input_text, return_tensors="np", padding=True, truncation=True)["input_ids"]
        # I don't add the BOS token here. 
        ...
        return self.last_obs, {} # return observation and info=None
    
    def _get_obs(self):
        decoded_state = self.tokenizer.decode(self.current_state)
        reward = self.reward(decoded_state, self.output_text)
...
if __name__ == "__main__":
    ...
    env = LanguageModelEnv(reward, tokenizer, [eos_token_id], 1024, dataset)
